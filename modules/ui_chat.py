    ...
    with gr.Tab('聊天', elem_id='chat-tab', elem_classes=("old-ui" if shared.args.chat_buttons else None)):
            ...
            with gr.Column(elem_id='chat-col'):
                ...
                with gr.Row(elem_id="chat-input-row"):
                    ...
                    with gr.Column(scale=10, elem_id='chat-input-container'):
                        ...
                        shared.gradio['show_controls'] = gr.Checkbox(value=shared.settings['show_controls'], label='显示控制 (Ctrl+S)', elem_id='show-controls')
        ...
        with gr.Column(elem_id='chat-buttons'):
            with gr.Row():
                ...
                shared.gradio['Remove last'] = gr.Button('删除最后回复 (Ctrl + Shift + Backspace)', elem_id='Remove-last')

            with gr.Row():
                shared.gradio['Replace last reply'] = gr.Button('替换最后回复 (Ctrl + Shift + L)', elem_id='Replace-last')
                shared.gradio['Copy last reply'] = gr.Button('复制最后回复 (Ctrl + Shift + K)', elem_id='Copy-last')
                shared.gradio['Impersonate'] = gr.Button('扮演 (Ctrl + Shift + M)', elem_id='Impersonate')

            with gr.Row():
                shared.gradio['Send dummy message'] = gr.Button('发送模拟消息')
                shared.gradio['Send dummy reply'] = gr.Button('发送虚假回复')

            with gr.Row():
                shared.gradio['send-chat-to-default'] = gr.Button('发送至Default')
                shared.gradio['send-chat-to-notebook'] = gr.Button('发送至Notebook')

        with gr.Row(elem_id='past-chats-row', elem_classes=['pretty_scrollbar']):
            with gr.Column():
                with gr.Row():
                    shared.gradio['unique_id'] = gr.Dropdown(label='聊天历史', elem_classes=['slim-dropdown'], interactive=not mu)

                with gr.Row():
                    shared.gradio['rename_chat'] = gr.Button('重命名', elem_classes='refresh-button', interactive=not mu)
                    shared.gradio['delete_chat'] = gr.Button('删除', elem_classes='refresh-button', interactive=not mu)
                    ...
                    shared.gradio['Start new chat'] = gr.Button('新对话', elem_classes='refresh-button')

                with gr.Row(elem_id='rename-row'):
                    shared.gradio['rename_to'] = gr.Textbox(label='重命名为:', placeholder='New name', visible=False, elem_classes=['no-background'])
        ...
        with gr.Row(elem_id='chat-controls', elem_classes=['pretty_scrollbar']):
            with gr.Column():
                with gr.Row():
                    shared.gradio['start_with'] = gr.Textbox(label='以...开始回复', placeholder='没问题!', value=shared.settings['start_with'], elem_classes=['add_scrollbar'])

                with gr.Row():
                    shared.gradio['mode'] = gr.Radio(choices=['聊天', '聊天-指令', '指令'], value='chat', label='模式', info='定义了聊天提示是如何生成的。在指令和聊天指令模式下，参数 > 指令模板中选择的指令模板必须与当前模型相匹配。', elem_id='chat-mode')

                with gr.Row():
                    shared.gradio['chat_style'] = gr.Dropdown(choices=utils.get_available_chat_styles(), label='聊天风格', value=shared.settings['chat_style'], visible=shared.settings['mode'] != 'instruct')
    ...
    with gr.Tab('聊天'):
        with gr.Row():
            with gr.Column(scale=8):
                with gr.Tab("角色"):
                    with gr.Row():
                        shared.gradio['character_menu'] = gr.Dropdown(value=None, choices=utils.get_available_characters(), label='角色', elem_id='character-menu', info='在聊天和聊天指令模式下使用', elem_classes='slim-dropdown')
                    ...
                    shared.gradio['name2'] = gr.Textbox(value='', lines=1, label='角色名称')
                    shared.gradio['context'] = gr.Textbox(value='', lines=10, label='上下文模板', elem_classes=['add_scrollbar'])
                    shared.gradio['greeting'] = gr.Textbox(value='', lines=5, label='问候', elem_classes=['add_scrollbar'])

                with gr.Tab("用户"):
                    shared.gradio['name1'] = gr.Textbox(value=shared.settings['name1'], lines=1, label='你的名称')
                    shared.gradio['user_bio'] = gr.Textbox(value=shared.settings['user_bio'], lines=10, label='描述', info='在这里，您可以写一段关于自己的描述', placeholder='{{user}}\'s personality: ...', elem_classes=['add_scrollbar'])

                with gr.Tab('聊天历史'):
                    with gr.Row():
                        with gr.Column():
                            shared.gradio['save_chat_history'] = gr.Button(value='保存历史')

                        with gr.Column():
                            shared.gradio['load_chat_history'] = gr.File(type='binary', file_types=['.json', '.txt'], label='上传历史JSON')

                with gr.Tab('上传角色文件'):
                    with gr.Tab('YAML or JSON'):
                        with gr.Row():
                            shared.gradio['upload_json'] = gr.File(type='binary', file_types=['.json', '.yaml'], label='JSON or YAML文件', interactive=not mu)
                            shared.gradio['upload_img_bot'] = gr.Image(type='pil', label='个人资料图片（可选）', interactive=not mu)
                    ...
                    with gr.Tab('TavernAI PNG'):
                        with gr.Row():
                            with gr.Column():
                                shared.gradio['upload_img_tavern'] = gr.Image(type='pil', label='TavernAI PNG文件', elem_id='upload_img_tavern', interactive=not mu)
            ...
            with gr.Column(scale=1):
                ...
                shared.gradio['your_picture'] = gr.Image(label='你的头像', type='pil', value=Image.open(Path('cache/pfp_me.png')) if Path('cache/pfp_me.png').exists() else None, interactive=not mu)

    with gr.Tab('指令模板'):
        with gr.Row():
            with gr.Column():
                with gr.Row():
                    shared.gradio['instruction_template'] = gr.Dropdown(choices=utils.get_available_instruction_templates(), label='保存对话模板', info="在选择模板后，点击“加载”以载入并应用它", value='None', elem_classes='slim-dropdown')
        ...
        with gr.Row():
            with gr.Column():
                shared.gradio['custom_system_message'] = gr.Textbox(value=shared.settings['custom_system_message'], lines=2, label='自定义系统消息', info='如果不为空，将代替默认设置使用。', elem_classes=['add_scrollbar'])
                shared.gradio['instruction_template_str'] = gr.Textbox(value='', label='对话模板', lines=24, info='根据您使用的模型/LoRA进行更改,在指令和聊天指令模式下使用', elem_classes=['add_scrollbar', 'monospace'])
                with gr.Row():
                    shared.gradio['send_instruction_to_default'] = gr.Button('发送至Default', elem_classes=['small-button'])
                    shared.gradio['send_instruction_to_notebook'] = gr.Button('发送至Notebook', elem_classes=['small-button'])
                    shared.gradio['send_instruction_to_negative_prompt'] = gr.Button('发送至negative prompt', elem_classes=['small-button'])
