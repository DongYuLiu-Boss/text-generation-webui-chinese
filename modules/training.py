...
def create_ui():
    ...
    with gr.Tab("训练", elem_id="training-tab"):
        with gr.Tab('训练LoRA', elem_id='lora-train-tab'):
            ...
            with gr.Row():
                with gr.Column():
                    gr.Markdown("[教程](https://github.com/oobabooga/text-generation-webui/wiki/05-%E2%80%90-Training-Tab)")

                    with gr.Row():
                        copy_from = gr.Dropdown(label='从…复制参数', value='None', choices=utils.get_available_loras(), elem_classes=['slim-dropdown'], interactive=not mu)
                    ...
                    with gr.Row():
                        with gr.Column(scale=5):
                            lora_name = gr.Textbox(label='名称', info='您创建的LoRA文件的名称')
                        with gr.Column():
                            always_override = gr.Checkbox(label='Override Existing Files', value=False, info='如果文件名相同，勾选将替换现有文件，取消勾选将从其中加载并继续（排名必须相同）。', elem_classes=['no-background'])

                    with gr.Accordion(label='目标模块', open=False):
                        gr.Markdown("选择在训练时针对哪些模块。针对更多模块接近于完全微调，但会增加VRAM需求和适配器大小。 注意：仅适用于model_id=‘llama’，其他类型将保持默认训练行为，不使用这些设置。")
                        with gr.Row():
                            with gr.Column():
                    ...
                    with gr.Row():
                        with gr.Column():
                            lora_rank = gr.Slider(label='LoRA Rank(训练范围)', value=32, minimum=0, maximum=1024, step=4, info='也称为维度计数。 higher values = 更大的文件，更多的内容控制。 Smaller values = 更小的文件，更少的控制。用于风格的值为4或8，用于教学的值为128或256，用于大数据精细细节的值为1024+。更高的等级需要更多的VRAM。')
                            lora_alpha = gr.Slider(label='LoRA Alpha', value=64, minimum=0, maximum=2048, step=4, info='这个值除以等级成为LoRA的缩放比例。数值越高意味着越强。一个好的标准值是你的等级的两倍。')
                            batch_size = gr.Slider(label='Batch Size', value=128, minimum=0, maximum=1024, step=4, info='全局批量大小。这两个批量大小共同决定了梯度累积（gradientAccum = batch / microBatch）。更高的梯度累积值会导致更好的训练质量。')
                            micro_batch_size = gr.Slider(label='Micro Batch Size', value=4, minimum=1, maximum=128, step=1, info='每个设备批量大小（注意：多设备尚未实现）。增加此值将增加VRAM使用量。')
                            cutoff_len = gr.Slider(label='Cutoff Length', minimum=0, maximum=4096, value=256, step=32, info='文本输入的截断长度。本质上，一次输入多少长度的文本行。更高的值需要大量的VRAM。')

                        with gr.Column():
                            save_steps = gr.Number(label='Save every n steps(保存LoRA备份相关设置)', value=0, info='如果大于0，每当经过这么多步时，就会保存一次LoRA的检查点。')

                            epochs = gr.Number(label='Epochs(训练单位)', value=3, info='数据集中的每个条目应该被输入训练中的次数。所以1意味着每个项目输入一次，5意味着输入五次，依此类推。')
                            learning_rate = gr.Textbox(label='Learning Rate(学习率)', value='3e-4', info='以科学记数法表示。3e-4是一个不错的起始基本点。1e-2非常高，1e-6非常低。')
                            with gr.Row():
                                lr_scheduler_type = gr.Dropdown(label='LR Scheduler(学习率调度器)', value='linear', choices=['linear', 'constant', 'constant_with_warmup', 'cosine', 'cosine_with_restarts', 'polynomial', 'inverse_sqrt'], info='学习率调度器 - 定义了学习率随时间如何变化。"常数"意味着从不改变， "线性"意味着学习率以直线方式从初始值下降到0，余弦波则遵循曲线等。', elem_classes=['slim-dropdown'])

                    with gr.Accordion(label='高级选项', open=False):
                        with gr.Row():
                            with gr.Column():
                                lora_dropout = gr.Slider(label='LoRA Dropout', minimum=0.0, maximum=1.0, step=0.025, value=0.05, info='LoRA层的损失概率百分比。这可以帮助减少过拟合。大多数用户应该保持默认值。')
                                stop_at_loss = gr.Slider(label='Stop at loss', minimum=0.0, maximum=3.0, step=0.1, value=0.00, info='一旦达到预期的损失值，该过程将自动停止。（合理的数值是1.5-1.8）')
                                with gr.Row():
                                    optimizer = gr.Dropdown(label='优化器', value='adamw_torch', choices=['adamw_hf', 'adamw_torch', 'adamw_torch_fused', 'adamw_torch_xla', 'adamw_apex_fused', 'adafactor', 'adamw_bnb_8bit', 'adamw_anyprecision', 'sgd', 'adagrad'], info='不同的优化器实现选项，适用于高级用户。不同选项的效果目前尚未很好地文档化。', elem_classes=['slim-dropdown'])

                            with gr.Column():
                                warmup_steps = gr.Number(label='预热步数', value=100, info='在开始时，学习率将低于正常值，持续这么多的步数。这有助于训练器准备模型并在开始后预计算统计信息，以提高训练质量。')
                                train_only_after = gr.Textbox(label='训练仅在…之后', value='', info='在给定块中，只考虑在此字符之后的所有文本用于训练。对于Alpaca数据集，使用"### Response:"来仅训练回复并忽略输入。')

                                add_eos_token = gr.Checkbox(label='添加EOS标记', value=False, info="为每个数据集项目添加EOS标记。在原始文本的情况下，EOS将在硬切处添加。")

                                higher_rank_limit = gr.Checkbox(label='激活更高的等级', value=False, info='如果勾选，将调整上面的Rank/Alpha滑块以达到更高的值。如果没有数据中心级别的GPU，这将无法工作。')
                                report_to = gr.Radio(label="保存详细日志", value="None", choices=["None", "wandb", "tensorboard"], interactive=True)
                ...
                with gr.Column():
                    with gr.Tab(label='格式化数据集'):
                        ...
                        with gr.Row():
                            eval_dataset = gr.Dropdown(choices=utils.get_datasets('training/datasets', 'json'), value='None', label='评估数据集', info='训练后用于评估模型的（可选）数据集文件', elem_classes=['slim-dropdown'], interactive=not mu)
                        ...
                        eval_steps = gr.Number(label='每N步评估', value=100, info='如果给出了评估数据集，每次经过这么多步骤时就对其进行测试')
                    ...
                    with gr.Tab(label="原始文本文件"):
                        ...
                        with gr.Row():
                            with gr.Column():
                                overlap_len = gr.Slider(label='重叠长度', minimum=0, maximum=512, value=128, step=16, info='从前一个文本块中包含多少个标记到下一个文本块中。（这些文本块本身的大小将由截止长度决定。）将重叠设置为截止长度的确切一半可能是最理想的。')
                                newline_favor_len = gr.Slider(label='换行符切分长度', minimum=0, maximum=512, value=128, step=16, info='以字符为单位，用于确保按换行符切割文本块时，重叠切割可以的最大位移距离。如果设置得太低，切割可能会出现在行的中间。')

                            with gr.Column():
                                hard_cut_string = gr.Textbox(label='硬切字符串', value='\\n\\n\\n', info='用于指示文本部分之间的硬切分的字符串。有助于防止不必要的重叠。')
                                min_chars = gr.Number(label='忽略小块', value=0, info='忽略字符数小于或等于这个数字的硬切块')

                    with gr.Row():
                        start_button = gr.Button("开始LoRA训练", variant='primary', interactive=not mu)
                        stop_button = gr.Button("中断", interactive=not mu)

                    output = gr.Markdown(value="准备好了")

        with gr.Tab('模型评估', elem_id='evaluate-tab'):
            with gr.Row():
                with gr.Column():
                    models = gr.Dropdown(utils.get_available_models(), label='模型', multiselect=True, interactive=not mu)
                    evaluate_text_file = gr.Dropdown(choices=['wikitext', 'ptb', 'ptb_new'] + utils.get_datasets('training/datasets', 'txt')[1:], value='wikitext', label='输入数据集', info='模型将被评估的原始文本文件。第一个选项会自动下载：wikitext、ptb 和 ptb_new。接下来的选项是你的本地文本文件，位于 training/datasets 目录下', interactive=not mu)
                    with gr.Row():
                        with gr.Column():
                            stride_length = gr.Slider(label='调整评估速度', minimum=0, maximum=32768, value=512, step=256, info='用于以牺牲准确度为代价来加快评估速度。1 = 最慢但最准确。512 是一个常见的值')

                        with gr.Column():
                            max_length = gr.Slider(label='max_length', minimum=0, maximum=shared.settings['truncation_length_max'], value=0, step=256, info='每次评估的上下文。如果设置为 0，将使用模型的最大上下文长度')

                    with gr.Row():
                        start_current_evaluation = gr.Button("评估已加载的模型", interactive=not mu)
                        start_evaluation = gr.Button("评估选定的模型", interactive=not mu)
            ...
            with gr.Row():
                save_comments = gr.Button('保存评估', elem_classes="small-button", interactive=not mu)
