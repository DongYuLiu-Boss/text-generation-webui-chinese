                ...
                with gr.Tab('逻辑值'):
                    with gr.Row():
                        with gr.Column(scale=10):
                            shared.gradio['get_logits-default'] = gr.Button('获取下一个令牌的概率')
                    ...
                    with gr.Row():
                        ...
                        shared.gradio['logits-default-previous'] = gr.Textbox(lines=23, label='上一个输出', elem_classes=['textbox_logits', 'add_scrollbar'])

                with gr.Tab('Tokens'):
                    shared.gradio['get_tokens-default'] = gr.Button('获取输入的令牌ID')
