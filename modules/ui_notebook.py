        ...
        with gr.Row():
            with gr.Column(scale=4):
                ...
                with gr.Tab('逻辑值'):
                    with gr.Row():
                        with gr.Column(scale=10):
                            shared.gradio['get_logits-notebook'] = gr.Button('获取下一个令牌的概率')
                    ...
                    with gr.Row():
                        ...
                        shared.gradio['logits-notebook-previous'] = gr.Textbox(lines=23, label='上一个输出', elem_classes=['textbox_logits_notebook', 'add_scrollbar'])

                with gr.Tab('Tokens'):
                    shared.gradio['get_tokens-notebook'] = gr.Button('获取输入的令牌ID')
                ...
                with gr.Row():
                    ...
                    shared.gradio['Undo'] = gr.Button('撤回', elem_classes='small-button')
                    shared.gradio['Regenerate-notebook'] = gr.Button('重新生成', elem_classes='small-button')

            with gr.Column(scale=1):
                gr.HTML('<div style="padding-bottom: 13px"></div>')
                with gr.Row():
                    shared.gradio['prompt_menu-notebook'] = gr.Dropdown(choices=utils.get_available_prompts(), value='None', label='提示', elem_classes='slim-dropdown')
